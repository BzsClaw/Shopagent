...
**影响范围**：i18n 文件 (18 个 locale)、daemon `server.ts`、`chat-routes.ts`、`Codex-diagnostics.ts`、`artifacts-cli.ts`、web 组件 (`HomeHero`、`HomeView`、`AvatarMenu`、`AssistantMessage`、`api-attachment-context` 等)
...
**Design 模式**需要本地 Agent CLI（如 Codex）才能工作。Design 模式 spawn 本地进程来创建/修改文件。

**启用条件**：
1. 至少一个 Agent CLI 已安装并可用（运行 `Codex --version` 验证）
2. Agent 已认证（Codex 需要 OAuth 登录或 API Key）
...
**当前可用 Agent**：`Codex`（通过 DeepSeek Anthropic API 中转）、`codex`、`hermes`
