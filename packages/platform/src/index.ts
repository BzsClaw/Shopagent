...
function defaultSystemProxyCommandRunner(command: string, args: string[]): string {
  return execFileSync(command, args, {
    ...
    // Raised from 2s → 5s to tolerate system load spikes (Astrill/VPN
    // proxies often cause reg.exe to block briefly on Windows I/O).
    timeout: 5_000,
    windowsHide: true,
  });
}
...
// Cache system proxy resolution for 30s. Each agent spawn previously
// ran 3 `reg query` / `scutil` calls, adding ~6s worst-case latency.
// Proxy settings rarely change at runtime so a short TTL is safe.
let _cachedSystemProxyEnv: { at: number; env: NodeJS.ProcessEnv } | null = null;
...
export function resolveSystemProxyEnv(options: ResolveSystemProxyEnvOptions = {}): NodeJS.ProcessEnv {
  // Only cache when no custom options are passed (the 99% case).
  const hasCustomOptions = options.platform != null || options.runCommand != null;
  if (!hasCustomOptions && _cachedSystemProxyEnv && Date.now() - _cachedSystemProxyEnv.at < SYSTEM_PROXY_CACHE_TTL_MS) {
    return _cachedSystemProxyEnv.env;
  }
  ...
  const tryRun = (command: string, args: string[]): string => {
    ...
    try {
      ...
      // Retry once — reg.exe can fail transiently under system load
      // (e.g. Astrill VPN reconnecting or I/O contention).
      try {
        return runCommand(command, args);
      } catch {
        return "";
  ...
  try {
    if (platform === "darwin") {
      result = parseMacosScutilProxyOutput(tryRun("scutil", ["--proxy"]), platform);
    } else if (platform === "win32") {
      result = parseWindowsInternetSettingsProxyOutput(
        {
    ...
    // Returns {} on failure — caller handles missing proxy gracefully.
  }
  if (!hasCustomOptions) {
    _cachedSystemProxyEnv = { at: Date.now(), env: result };
  }
  return result;
}
