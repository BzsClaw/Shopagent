...
// Always exclude localhost from proxy to prevent daemon-to-daemon
// loopback from going through the VPN (adds latency + instability).
const NO_PROXY_LOOPBACK = ['localhost', '127.0.0.1', '[::1]'];

function ensureNoProxyLoopback(env: NodeJS.ProcessEnv): void {
  // Only relevant when a proxy endpoint is configured.
  const hasProxy = ['HTTP_PROXY', 'HTTPS_PROXY', 'ALL_PROXY'].some((k) =>
    Object.keys(env).some((ek) => ek.toUpperCase() === k && env[ek]?.trim()),
  );
  if (!hasProxy) return;
  // Pick the canonical NO_PROXY key (case-insensitive).
  const existingKey = Object.keys(env).find(
    (k) => k.toUpperCase() === 'NO_PROXY',
  );
  const raw = existingKey ? (env[existingKey] ?? '') : '';
  const existing = raw
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  const missing = NO_PROXY_LOOPBACK.filter(
    (host) => !existing.includes(host),
  );
  if (missing.length === 0) return;
  if (existingKey) delete env[existingKey];
  env.NO_PROXY = [...existing, ...missing].join(',');
}
...
export function spawnEnvForAgent(
  ...
  // Ensure daemon loopback never goes through the proxy — even if
  // system NO_PROXY is missing or process.env overrides it away.
