...
export function parseRoute(pathname: string): Route {
  ...
  if (parts[0] === 'workflow') {
    if (parts[1]) {
      return { kind: 'workflow-detail', workflowId: decodeURIComponent(parts[1]) };
    }
...
export function buildPath(route: Route): string {
  ...
  if (route.kind === 'listing') return '/listing';
  if (route.kind === 'methodology') return '/methodology';
  if (route.kind === 'workflow') return '/workflow';
  if (route.kind === 'workflow-detail') return `/workflow/${encodeURIComponent(route.workflowId)}`;
