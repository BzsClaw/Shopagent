      ...
      setNotice({
        message:
          action === "publish-github"
            ...
            : `Open Yourself contribution task started for ${data?.path ?? "the draft"}.`,
      });
    } catch (err) {
      setNotice({ message: err instanceof Error ? err.message : String(err) });
    } finally {
...
interface Props {
  ...
  // Click handler for the "Share to Open Yourself" button rendered next to
      ...
      <div className="assistant-flow">
        ...
        {showCompletionRow ? (
          <div className="assistant-completion-row">
            ...
            {/*
              "Share to Open Yourself" — pairs with the post-feedback Discord
        ...
        <div className="assistant-feedback-reasons" ref={reasonsRef}>
          ...
          {reasonRating === "positive" ? (
            <p className="assistant-feedback-discord-note">
              Thanks for the feedback! Your input helps us improve.
            </p>
          ) : (
            <p className="assistant-feedback-discord-note">
...
function hasPluginFinalActionHint(content: string): boolean {
  return /\b(Add to My plugins|Open Yourself PR|Publish repo|plugin publish|ready to publish|ready to add)\b/i.test(
    content,
  );
}
