...
function tabFromRoute(route: Route, timestamp = Date.now()): WorkspaceChromeTab {
  ...
  if (route.kind === 'listing') {
    return { id: `listing:${nowId()}`, kind: 'listing', createdAt: timestamp, lastActiveAt: timestamp };
  }
  if (route.kind === 'methodology') {
    return { id: `methodology:${nowId()}`, kind: 'methodology', createdAt: timestamp, lastActiveAt: timestamp };
  }
  if (route.kind === 'workflow') {
    return { id: `workflow:${nowId()}`, kind: 'workflow', createdAt: timestamp, lastActiveAt: timestamp };
  }
  if (route.kind === 'workflow-detail') {
    return { id: `workflow-detail:${nowId()}`, kind: 'workflow-detail', workflowId: route.workflowId, createdAt: timestamp, lastActiveAt: timestamp };
  }
...
function routeForTab(tab: WorkspaceChromeTab): Route {
  ...
  if (tab.kind === 'marketplace') {
    return tab.pluginId
	  ...
	  if (tab.kind === 'listing') return { kind: 'listing' };
	  if (tab.kind === 'methodology') return { kind: 'methodology' };
	  if (tab.kind === 'workflow') return { kind: 'workflow' };
	  if (tab.kind === 'workflow-detail') return { kind: 'workflow-detail', workflowId: tab.workflowId };
...
function reviveTab(value: unknown): WorkspaceChromeTab | null {
  ...
  if (record.kind === 'workflow') {
    return { id, kind: 'workflow', createdAt, lastActiveAt };
  }
  if (record.kind === 'workflow-detail') {
    return { id, kind: 'workflow-detail', workflowId: (record as any).workflowId ?? '', createdAt, lastActiveAt };
  }
  return null;
}
...
function uniqueIdForTab(tab: WorkspaceChromeTab): string {
  ...
  if (tab.kind === 'listing') return `listing:${nowId()}`;
  if (tab.kind === 'methodology') return `methodology:${nowId()}`;
  if (tab.kind === 'workflow') return `workflow:${nowId()}`;
  if (tab.kind === 'workflow-detail') return `workflow-detail:${nowId()}`;
...
function displayTabFor(
  ...
  if (tab.kind === 'workflow') return { id: tab.id, title: 'Workflow', meta: 'Workflow', icon: 'flow' as IconName, tab };
  if (tab.kind === 'workflow-detail') return { id: tab.id, title: 'Workflow', meta: 'Workflow', icon: 'flow' as IconName, tab };
