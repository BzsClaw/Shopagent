...
export function TasksView({ skills = [], designTemplates = [], connectors = [] }: Props) {
  ...
  const filterLabel: Record<string, string> = {
    all: t('tasksView.filterAll'),
    orbit: t('tasksView.filterOrbit'),
    'live-artifact': t('tasksView.filterLiveArtifact'),
    memory: t('tasksView.filterMemory'),
    'design-system': t('tasksView.filterDesignSystem'),
    skills: t('tasksView.filterSkills'),
    connectors: t('tasksView.filterConnectors'),
    compression: t('tasksView.filterCompression'),
    release: t('tasksView.filterRelease'),
    quality: t('tasksView.filterQuality'),
  };
    ...
    <section className="automations-view" aria-labelledby="automations-title" data-testid="tasks-view">
      <header className="automations-hero">
        <div className="automations-hero__copy">
          <span className="automations-hero__eyebrow">{t('tasksView.eyebrow')}</span>
        ...
        <div className="automations-hero__actions">
          <div className="automations-metrics" aria-label="Automation summary">
            <Metric label={t('tasksView.active')} value={activeCount} />
            <Metric label={t('tasksView.paused')} value={pausedCount} />
            <Metric label={t('tasksView.templates')} value={templates.length} />
          </div>
          <button
            type="button"
        ...
        {!loading && sortedRoutines.length === 0 ? (
          ...
          >
            ...
            <span className="automation-empty__body">
              <strong>{t('tasksView.emptyTitle')}</strong>
              <span>{t('tasksView.emptyDesc')}</span>
            </span>
          </button>
        ) : null}
                    ...
                    <span className="automation-row__content">
                      ...
                      {r.lastRun ? (
                        <span className="automation-row__last-run">
                          ...
                          <span>{t('tasksView.lastRun', { time: formatAutomationTimestamp(r.lastRun.startedAt) })}</span>
                          <span aria-hidden="true">·</span>
                          <button
                            type="button"
      ...
      <section className="automations-templates" aria-label="Automation templates">
        ...
        >
          {TEMPLATE_FILTERS.map((filter) => {
            ...
            return (
              ...
              >
                <span className="automations-template-tab__label">{filterLabel[filter.id] ?? filter.label}</span>
        ...
        {filteredTemplates.length === 0 ? (
          <div className="automations-templates__empty" role="status">
            ...
            <div>
              <strong>{t('tasksView.noTemplatesTitle')}</strong>
              <p>{t('tasksView.noTemplatesDesc')}</p>
            </div>
          </div>
        ) : null}
  ...
  return (
    <div className="automation-history" aria-label="Automation run history">
      <div className="automation-history__head">
        <span>{t('tasksView.runHistory')}</span>
        <span>{t('tasksView.latest', { n: '10' })}</span>
      </div>
